...
func (h *Hub) run() {
	...
	for {
		...
		case msg := <-h.route:
			...
			if dst := h.topicGet(msg.rcptto); dst != nil {
				...
				if dst.broadcast != nil {
					...
					default:
						h.logger.Info("hub: topic's broadcast queue is full ", dst.name)
					}
				}
				...
				h.logger.Infof("Hub. Topic[%s] is unknown or offline", msg.rcptto)
		...
		case unreg := <-h.unreg:
			h.logger.Info("hub.run unreg := <-h.unreg:")
			reason := StopNone
			if unreg.del {
				reason = StopDeleted
			}
			if unreg.forUser.IsZero() {
				...
				if err := h.topicUnreg(unreg.sess, unreg.topic, unreg.pkt, reason); err != nil {
					h.logger.Info("hub.topicUnreg failed:", err)
				}
			} else {
		...
		case <-h.rehash:
			h.topics.Range(func(_, t interface{}) bool {
				topic := t.(*Topic)
				if globals.cluster.isRemoteTopic(topic.name,topic.name) {
		...
		case hubdone := <-h.shutdown:
			...
			h.logger.Infof("Hub shutdown completed with %d topics", topicCount)
		...
		case sget := <-h.get:
			h.logger.Infof("hub get topic %s.", sget.topic)
			t := h.topicGet(sget.topic)
			if t == nil {
				sget.existed <- false
			}else {
				sget.existed <- true
...
func topicInit(sreg *sessionJoin, h *Hub) {
	...
	parseMode := func(modeString string, defaultMode types.AccessMode) types.AccessMode {
		...
		if err := mode.UnmarshalText([]byte(modeString)); err != nil {
			h.logger.Info("hub: invalid access mode for topic[" + t.xoriginal + "]: '" + modeString + "'")
		}
	...
	if t.xoriginal == "me" {
		...
		if err != nil {
			h.logger.Info("hub: cannot load 'me' user object", t.name, err)
			// Log out the session
			sreg.sess.uid = types.ZeroUid
			sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
			return
		} else if user == nil {
			h.logger.Info("hub: user's account not found", t.name)
			// Log out the session
		...
		if err = t.loadSubscribers(); err != nil {
			h.logger.Info("hub: cannot load subscribers for '" + t.name + "' (" + err.Error() + ")")
			sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
			return
		}
		...
		user, err := store.Users.Get(sreg.sess.uid)
		if err != nil {
			h.logger.Info("hub: cannot load user object for 'fnd'='" + t.name + "' (" + err.Error() + ")")
			sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
			return
		} else if user == nil {
			h.logger.Info("hub: user's account unexpectedly not found (deleted?)")
			// FIXME: this is a race condition
			sreg.sess.uid = types.ZeroUid
			sreg.sess.queueOut(ErrUserNotFound(sreg.pkt.id, t.xoriginal, timestamp))
			return
		...
		if err != nil {
			h.logger.Info("hub: error while loading topic '" + t.name + "' (" + err.Error() + ")")
			sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
			return
		}
		...
		if stopic != nil {
			...
			if subs, err = store.Topics.GetUsers(t.name, nil); err != nil {
				h.logger.Info("hub: cannot load subscriptions for '" + t.name + "' (" + err.Error() + ")")
				sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
				return
			}
			...
			// Case 3, fail
			if len(subs) == 0 {
				h.logger.Info("hub: missing both subscriptions for '" + t.name + "' (SHOULD NEVER HAPPEN!)")
				sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
				return
			}
		...
		if stopic != nil && len(subs) == 2 {
			...
			if err != nil {
				h.logger.Info("hub: failed to load users for '" + t.name + "' (" + err.Error() + ")")
				sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
				return
			}
			if len(users) == 0 {
				// Invited user does not exist
				h.logger.Info("hub: missing user for '" + t.name + "'")
				sreg.sess.queueOut(ErrUserNotFound(sreg.pkt.id, t.xoriginal, timestamp))
				return
			}
				...
				if pktsub.Set != nil {
					if pktsub.Set.Sub != nil {
						...
						if uid != userID1 {
							...
							h.logger.Info("hub: setting mode for another user is not supported '" + t.name + "'")
						} else {
			...
			if stopic == nil {
				if err = store.Topics.CreateP2P(sub1, sub2); err != nil {
					h.logger.Info("hub: databse error in creating subscriptions '" + t.name + "' (" + err.Error() + ")")
					sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
					return
				}
				...
				if err = store.Subs.Create(subToMake); err != nil {
					h.logger.Info("hub: databse error in re-subscribing user '" + t.name + "' (" + err.Error() + ")")
					sreg.sess.queueOut(ErrUnknown(sreg.pkt.id, t.xoriginal, timestamp))
					return
				}
			}
		...
		//if t.name start with grp
		// CreateClusterTopic
		if strings.Contains(t.name, "grp") {
			//_host, _port := globals.cluster.GetClusterHost()
			if (globals.cluster != nil){
				t.cluster = NewClusterTopic(t.name,globals.cluster.thisNodeName, globals.cluster.listenPort, false,"unknow")
			}
		}
		...
		if pktsub.Set != nil {
			...
			if pktsub.Set.Desc != nil {
				...
				if pktsub.Set.Desc.DefaultAcs != nil {
					if authMode, anonMode, err := parseTopicAccess(pktsub.Set.Desc.DefaultAcs,
						...
						h.logger.Info("hub: invalid access mode for topic '" + t.name + "': '" + err.Error() + "'")
					} else if authMode.IsOwner() || anonMode.IsOwner() {
						h.logger.Info("hub: OWNER default access in topic '" + t.name)
			...
			if !restrictedTagsEqual(tags, nil, globals.immutableTagNS) {
				h.logger.Info("hub: attempt to directly set restricted tags")
		...
		if err != nil {
			h.logger.Info("hub: cannot save new topic '" + t.name + "' (" + err.Error() + ")")
		...
		if err != nil {
			...
			h.logger.Info("hub: topic '" + t.name + "' does not exist")
...
func (h *Hub) stopTopicsForUser(uid types.Uid, reason int, alldone chan<- bool) {
	defer TryLogDump()
	h.logger.Info("stopTopicsForUser in")
	defer TryLogDump()
	var done chan bool
	if alldone != nil {
...
func replyOfflineTopicGetDesc(sess *Session, topic string, msg *ClientComMessage) {
	...
	if strings.HasPrefix(topic, "grp") {
		...
		if err != nil {
			sess.logger.Info("replyOfflineTopicGetDesc", err)
		...
		if uid.IsZero() {
			sess.logger.Info("replyOfflineTopicGetDesc: malformed p2p topic name")
...
func replyOfflineTopicGetSub(sess *Session, topic string, msg *ClientComMessage) {
	...
	if err != nil {
		sess.logger.Info("replyOfflineTopicGetSub:", err)
		sess.queueOut(decodeStoreError(err, msg.id, msg.topic, now, nil))
		return
	}
...
func replyOfflineTopicSetSub(sess *Session, topic string, msg *ClientComMessage) {
	...
	sub, err := store.Subs.Get(topic, asUid)
	if err != nil {
		sess.logger.Info("replyOfflineTopicSetSub get sub:", err)
		sess.queueOut(decodeStoreError(err, msg.id, msg.topic, now, nil))
		return
	}
	...
	if msg.Set.Sub != nil && msg.Set.Sub.Mode != "" {
		...
		if err = modeWant.UnmarshalText([]byte(msg.Set.Sub.Mode)); err != nil {
			sess.logger.Info("replyOfflineTopicSetSub mode:", err)
			sess.queueOut(decodeStoreError(err, msg.id, msg.topic, now, nil))
			return
		}
	...
	if len(update) > 0 {
		...
		if err != nil {
			sess.logger.Info("replyOfflineTopicSetSub update:", err)
			sess.queueOut(decodeStoreError(err, msg.id, msg.topic, now, nil))
		} else {
