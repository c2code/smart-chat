...
func (t *Topic) run(hub *Hub) {
	...
	for {
			...
			if t.isSuspended() {
				...
				if err := t.handleSubscription(hub, sreg); err == nil {
					...
					if len(t.sessions) == 0 {
						...
						globals.logger.Info("topic reset KillTimer.")
						killTimer.Reset(keepAlive)
					}
					t.logger.Infof("topic[%s] subscription failed %v", t.name, err)
				}
			}
		...
		case leave := <-t.unreg:
			...
			asUid := leave.userId
			t.logger.Infof("t.run leave := <-t.unreg,asUid=%v", asUid)
			if t.isSuspended() {
				...
				if err := t.replyLeaveUnsub(hub, leave.sess, asUid, leave.reqID); err != nil {
					t.logger.Info("failed to unsub", err)
					continue
				}
		...
		case msg := <-t.broadcast:
			...
			if msg.Data != nil {
				...
				if err := store.Messages.Save(&types.Message{
					...
					t.logger.Infof("topic[%s]: failed to save message: %v", t.name, err)
				...
				if msg.Info.What == "read" || msg.Info.What == "recv" {
					...
					if err := store.Subs.Update(t.name, uid,
						...
						t.logger.Infof("topic[%s]: failed to update SeqRead/Recv counter: %v", t.name, err)
						continue
					}
			...
			if msg.Data != nil || msg.Pres != nil || msg.Info != nil {
				for sess := range t.sessions {
					...
					if sess.queueOut(msg) {
						...
						t.logger.Infof("topic[%s]: connection stuck, detaching", t.name)
				...
				t.logger.Panic("topic: wrong message type for broadcasting", t.name)
			}
			...
			case meta.pkt.Set != nil:
				...
				if meta.what&constMsgMetaDesc != 0 {
					if err := t.replySetDesc(meta.sess, asUid, meta.pkt.Set); err == nil {
						...
						t.logger.Infof("topic[%s] meta.Set.Desc failed: %v", t.name, err)
					}
				}
				if meta.what&constMsgMetaSub != 0 {
					if err := t.replySetSub(hub, meta.sess, meta.pkt); err != nil {
						t.logger.Infof("topic[%s] meta.Set.Sub failed: %v", t.name, err)
					}
				}
				if meta.what&constMsgMetaTags != 0 {
					if err := t.replySetTags(meta.sess, asUid, meta.pkt.Set); err != nil {
						t.logger.Infof("topic[%s] meta.Set.Tags failed: %v", t.name, err)
					}
				}

			case meta.pkt.Del != nil:
				// Del request
				globals.logger.Infof("topic meta.pkg.Del %v what %v", meta.pkt.Del, meta.what)
				var err error
				switch meta.what {
				case constMsgDelMsg:
				...
				if err != nil {
					t.logger.Infof("topic[%s] meta.Del failed: %v", t.name, err)
				}
			}
		case ua := <-t.uaChange:
		...
		case <-killTimer.C:
			// Topic timeout
			t.logger.Infof("topic killTimer t.name %s", t.name)
			hub.unreg <- &topicUnreg{topic: t.name}
			if t.cat == types.TopicCatMe {
				uaTimer.Stop()
				t.presUsersOfInterest("off", currentUA)
				if t.me_cluserInfo != nil {
					t.me_cluserInfo.SetUserOffline()
				}
		...
		case sd := <-t.exit:
			...
			t.logger.Infof("sd := <-t.exit,sd.reason %v ,t.cat %v", sd.reason, t.cat)
...
func (t *Topic) handleSubscription(h *Hub, sreg *sessionJoin) error {
	...
	if getWhat&constMsgMetaDesc != 0 {
		...
		if err := t.replyGetDesc(sreg.sess, asUid, sreg.pkt.id, msgsub.Get.Desc); err != nil {
			t.logger.Infof("topic[%s] subCommonReply Get.Desc failed: %v", t.name, err)
		}
	}
	...
	if getWhat&constMsgMetaSub != 0 {
		...
		if err := t.replyGetSub(sreg.sess, asUid, authLevel, sreg.pkt.id, msgsub.Get.Sub); err != nil {
			t.logger.Infof("topic[%s] handleSubscription Get.Sub failed: %v", t.name, err)
		}
	}
	...
	if getWhat&constMsgMetaTags != 0 {
		...
		if err := t.replyGetTags(sreg.sess, asUid, sreg.pkt.id); err != nil {
			t.logger.Infof("topic[%s] handleSubscription Get.Tags failed: %v", t.name, err)
		}
	}
	...
	if getWhat&constMsgMetaData != 0 {
		...
		if err := t.replyGetData(sreg.sess, asUid, sreg.pkt.id, msgsub.Get.Data); err != nil {
			t.logger.Infof("topic[%s] handleSubscription Get.Data failed: %v", t.name, err)
		}
	}
	...
	if getWhat&constMsgMetaDel != 0 {
		...
		if err := t.replyGetDel(sreg.sess, asUid, sreg.pkt.id, msgsub.Get.Del); err != nil {
			t.logger.Infof("topic[%s] handleSubscription Get.Del failed: %v", t.name, err)
		}
	}
...
func (t *Topic) requestSub(h *Hub, sess *Session, asUid types.Uid, asLvl auth.Level,
	...
	if !existingSub || userData.deleted {
		t.logger.Infof("topic[%s] requestSub sub is nil", t.name)
		...
		t.logger.Infof("topic[%s] requestSub create sub user %s", t.name, sub.User)
		if err := store.Subs.Create(sub); err != nil {
			t.logger.Infof("topic[%s] requestSub create db fail", t.name)
		...
		t.logger.Infof("topic[%s] requestSub sub not nil", t.name)
		...
		if len(update) > 0 {
			t.logger.Infof("topic[%s] requestSub update sub user %s", t.name, asUid.String())
...
func (t *Topic) replyGetDesc(sess *Session, asUid types.Uid, id string, opts *MsgGetOpts) error {
	...
	if t.cat == types.TopicCatP2P && !full {
		t.logger.Info("missing p2p subscription in getDesc")
	}
	if t.cat == types.TopicCatMe {
		full = true
	}
...
func (t *Topic) replyGetSub(sess *Session, asUid types.Uid, authLevel auth.Level, id string, req *MsgGetOpts) error {
	...
	if len(subs) > 0 {
		...
		for i := range subs {
			...
			//mts.Topic = strings.TrimSpace(mts.Topic)
...
func getsSubGrptopic(originalTopic string) *string {
	//originalTopic subscription get grpxxxx topic
	subs, err := store.Topics.GetSubs(originalTopic, nil)
	if err != nil || subs == nil {
		return nil
	}
	for i := range subs {
		if strings.HasPrefix(subs[i].Topic, "grp") {
			grpTopic := &subs[i].Topic
			return grpTopic
		}
	}
	return nil
...
func (t *Topic) replyDelMsg(sess *Session, asUid types.Uid, del *MsgClientDel) error {
	...
	if del.Hard {
		...
		globals.logger.Infof("topic %v del msg %v", t.name, dr)
		...
		pud.delID = t.delID
		t.perUser[asUid] = pud
		globals.logger.Infof("topic %v soft del msg %v", t.name, dr)
		// Notify user's other sessions
...
func (t *Topic) notifySubChange(uid, actor types.Uid, oldWant, oldGiven,
	...
	if unsub {
		...
		// t.logger.Infof("topic[%s] sending ?unkn+en to me[%s]", t.name, asUid.String())
...
func (t *Topic) original(uid types.Uid) string {
	...
	t.logger.Panic("Invalid P2P topic")
	return ""
}
...
func (t *Topic) p2pOtherUser(uid types.Uid) types.Uid {
	...
	t.logger.Panic("Not a valid P2P topic")
	return 0
}
...
func (t *Topic) fndGetPublic(sess *Session) interface{} {
	if t.cat == types.TopicCatFnd {
		...
		t.logger.Panic("Invalid Fnd.Public type")
	}
	t.logger.Panic("Not Fnd topic")
	return nil
}
...
func (t *Topic) fndRemovePublic(sess *Session) {
	if t.cat == types.TopicCatFnd {
		...
		globals.logger.Panic("Invalid Fnd.Public type")
	}
	globals.logger.Panic("Not Fnd topic")
}
...
func (t *Topic) addSession(s *Session, user types.Uid) types.UidSlice {
	globals.logger.Infof("addSession sid %s, uid %v", s.sid, user)
	uids := t.sessions[s]
	if uids == nil {
...
func (t *Topic) remSession(s *Session, user types.Uid) types.UidSlice {
	...
	globals.logger.Infof("remSession sid %s, uid %v", s.sid, user)
	uids := t.sessions[s]
	if uids == nil {
		return nil
	}
