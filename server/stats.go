...
func statsInit(mux *http.ServeMux, path string) {
	...
	globals.logger.Infof("stats: variables exposed at '%s'", path)
}
...
func statsUpdater() {
	...
	for upd := range globals.statsUpdate {
		...
		if ev := expvar.Get(upd.varname); ev != nil {
			...
			globals.logger.Panic("stats: update to unknown variable " + upd.varname)
		}
	}

	globals.logger.Info("stats: shutdown")
}
