...
func (ug *UidGenerator) GetStrAndUid() (string ,Uid) {
	buf, err := getIDBuffer(ug)
	if err != nil {
		return "", ZeroUid
	}
	return base64.URLEncoding.WithPadding(base64.NoPadding).EncodeToString(buf),Uid(binary.LittleEndian.Uint64(buf))
}
