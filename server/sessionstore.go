...
func (ss *SessionStore) NewSession(conn interface{}, sid string) (*Session, int) {
	...
	s.logger = logp.NewLogger("session")
	globals.logger.Infof("NewSession sid %s", sid)
	return &s, count
}
...
func (ss *SessionStore) Delete(s *Session) {
	...
	globals.logger.Infof("Delete sid %s", s.sid)
	statsSet("LiveSessions", int64(len(ss.sessCache)))
}
...
func (ss *SessionStore) Shutdown() {
	...
	ss.logger.Infof("SessionStore shut down, sessions terminated: %d", len(ss.sessCache))
}
...
func (ss *SessionStore) EvictUser(uid types.Uid, skipSid string) {
	ss.lock.Lock()
	defer ss.lock.Unlock()
	ss.logger.Infof("SessionStore EvictUser: %v", uid)
...
func NewSessionStore(lifetime time.Duration) *SessionStore {
	...
	ss.logger = logp.NewLogger("SessionStore")
...
//EvictSession, only call when rpc node is reconnect
func (ss *SessionStore) EvictSession(nodeName string) {
	ss.lock.Lock()
	defer ss.lock.Unlock()
	ss.logger.Infof("SessionStore EvictSession, nodeName %s", nodeName)
	evicted := NoErrEvicted("", "", types.TimeNow())
	for _, s := range ss.sessCache {

		for addr, val := range s.nodes {
			if true == val {
				if strings.Compare(addr, nodeName) == 0 {
					if s.stop != nil {
						if s.proto == WEBSOCK {
							s.stop <- s.serialize(evicted)
							delete(ss.sessCache, s.sid)
