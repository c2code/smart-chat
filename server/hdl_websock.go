	...
	for {
		...
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure,
				...
				globals.logger.Warnf("ws: readLoop %s %s", sess.sid, err.Error())
			}
			return
		}
...
func (sess *Session) writeLoop() {
	...
	for {
		...
		case msg, ok := <-sess.send:
			...
			msg_bytes := msg.([]byte)
			log.Println("send:",string(msg_bytes[:]))
			if err := wsWrite(sess.ws, websocket.TextMessage, msg); err != nil {
				if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure,
					websocket.CloseNormalClosure) {
					globals.logger.Infof("ws: writeLoop %s %s", sess.sid, err.Error())
				}
				return
			}
		case msg := <-sess.stop:
		...
		case <-ticker.C:
			if err := wsWrite(sess.ws, websocket.PingMessage, nil); err != nil {
				if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure,
					...
					globals.logger.Infof("ws: writeLoop ping %s %s", sess.sid, err.Error())
				}
				return
			}
		}
...
func serveWebSocket(wrt http.ResponseWriter, req *http.Request) {
	...
	if isValid, _ := checkAPIKey(getAPIKey(req)); !isValid {
		...
		globals.logger.Warn("ws: Missing, invalid or expired API key")
		return
	}
	...
	if req.Method != http.MethodGet {
		...
		globals.logger.Warn("ws: Invalid HTTP method", req.Method)
		return
	}

	ws, err := upgrader.Upgrade(wrt, req, nil)
	if _, ok := err.(websocket.HandshakeError); ok {
		globals.logger.Warn("ws: Not a websocket handshake")
		return
	} else if err != nil {
		globals.logger.Warn("ws: failed to Upgrade ", err)
		return
	}

	sess, count := globals.sessionStore.NewSession(ws, "")

	globals.logger.Infof("ws: session started %s %d", sess.sid, count)
