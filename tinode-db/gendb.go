...
func genDb(reset bool, config string, data *Data) {
	...
	if err != nil {
		...
		// try to insert a sub into subscriptions
		/*genStr := store.GetUidString()
		topic_random := "grp" + genStr
		sub := &types.Subscription{
			User:      genStr,
			Topic:     topic_random,
			ModeWant:  types.ModeCPublic,
			ModeGiven: types.ModeCPublic,
		...
		if err := store.Subs.Create(sub); err != nil {
			...
			log.Println("Failed to insert subs:", err)
			return
		}

		store.Subs.DeleteHard(topic_random)*/
		return
	}
