...
func replyCreateUser(s *Session, msg *ClientComMessage, rec *auth.Rec) {
	...
	if msg.Acc.Login && (!s.uid.IsZero() || rec != nil) {
		...
		globals.logger.Infof("s.acc: login requested while authenticated %s", s.sid)
		return
	}
	...
	if authhdl == nil {
		...
		globals.logger.Infof("s.acc: unknown auth handler %s", s.sid)
		return
	}

	// Check if login is unique.
	if ok, err := authhdl.IsUnique(msg.Acc.Secret); !ok {
		globals.logger.Infof("s.acc: auth secret is not unique %s %s", err.Error(), s.sid)
	...
	user.Access.Anon = getDefaultAccess(types.TopicCatP2P, true)

	if tags := normalizeTags(msg.Acc.Tags); tags != nil {
		if !restrictedTagsEqual(tags, nil, globals.immutableTagNS) {
			globals.logger.Infof("a.acc: attempt to directly assign restricted tags %s", s.sid)
	...
	for i := range creds {
		...
		if err := vld.PreCheck(cr.Value, cr.Params); err != nil {
			globals.logger.Infof("a.acc: failed credential pre-check %s %s", err.Error(), s.sid)
	...
	_,uid := store.GetStrAndUid();
	user.SetUid(uid)
	if _, err := store.Users.Create(&user, private); err != nil {
		globals.logger.Infof("s.acc: failed to create user %s %s", err.Error(), s.sid)
	...
	if err != nil {
		globals.logger.Infof("s.acc: add auth record failed %s %s", err.Error(), s.sid)
	...
	for i := range creds {
		...
		if err := vld.Request(user.Uid(), cr.Value, s.lang, cr.Response, tmpToken); err != nil {
			globals.logger.Infof("s.acc: failed to save or validate credential %s %s", err.Error(), s.sid)
	...
	if err := store.Users.Update(user.Uid(),
		...
		globals.logger.Infof("s.acc: failed to update user's tags %s %s", err.Error(), s.sid)
		// Delete incomplete user record.
		store.Users.Delete(user.Uid(), false)
		s.queueOut(decodeStoreError(err, msg.id, "", msg.timestamp, nil))
		return
...
func replyUpdateUser(s *Session, msg *ClientComMessage, rec *auth.Rec) {
	if s.uid.IsZero() && rec == nil {
		...
		globals.logger.Infof("replyUpdateUser: not a new account and not authenticated %s", s.sid)
		...
		globals.logger.Infof("replyUpdateUser: got both authenticated session and token %s", s.sid)
		s.queueOut(ErrMalformed(msg.id, "", msg.timestamp))
		return
	}
	...
	if msg.Acc.User != "" && msg.Acc.User != userId {
		if s.authLvl != auth.LevelRoot {
			globals.logger.Infof("replyUpdateUser: attempt to change another's account by non-root %s", s.sid)
			s.queueOut(ErrPermissionDenied(msg.id, "", msg.timestamp))
			return
		}
	...
	if uid.IsZero() || authLvl == auth.LevelNone {
		...
		s.queueOut(ErrMalformed(msg.id, "", msg.timestamp))
		globals.logger.Infof("replyUpdateUser: either user id or auth level is missing %s", s.sid)
		return
	}
	...
	if err != nil {
		globals.logger.Infof("replyUpdateUser: failed to fetch user from DB %s %s", err.Error(), s.sid)
		s.queueOut(decodeStoreError(err, msg.id, "", msg.timestamp, nil))
		return
	}
	...
	if authhdl != nil {
		...
		if err != nil {
			globals.logger.Infof("replyUpdateUser: failed to update auth secret %s %s", err.Error(), s.sid)
			s.queueOut(decodeStoreError(err, msg.id, "", msg.timestamp, nil))
			return
		}
...
func replyDelUser(s *Session, msg *ClientComMessage) {
	...
	if msg.Del.User == "" || msg.Del.User == s.uid.UserId() {
		...
		if uid.IsZero() {
			...
			globals.logger.Infof("replyDelUser: invalid user ID %s %s", msg.Del.User, s.sid)
		}
	} else {
		reply = ErrPermissionDenied(msg.id, "", msg.timestamp)
		globals.logger.Infof("replyDelUser: illegal attempt to delete another user %s %s", msg.Del.User, s.sid)
	}
	...
	if reply == nil {
		...
		for _, name := range authnames {
			if err := store.GetAuthHandler(name).DelRecords(uid); err != nil {
				...
				globals.logger.Infof("replyDelUser: failed to delete auth record %s %s %s %s", uid.UserId(), name, err.Error(), s.sid)
			}
		}
		...
		if uoi, err := store.Users.GetSubs(uid, nil); err == nil {
			...
			globals.logger.Info("notifying users of interest")
			presUsersOfInterestOffline(uid, uoi, "gone")
		} else {
			globals.logger.Infof("replyDelUser: failed to send notifications to users %s %s", err.Error(), s.sid)
		}
		...
		if ownTopics, err := store.Users.GetOwnTopics(uid, nil); err == nil {
			globals.logger.Info("deleting owned topics ", ownTopics)
			...
			globals.logger.Infof("replyDelUser: failed to send notifications to owned topics %s %s", err.Error(), s.sid)
		}
		...
		if err := store.Users.Delete(uid, msg.Del.Hard); err != nil {
			...
			globals.logger.Infof("replyDelUser: failed to delete user %s %s", err.Error(), s.sid)
		} else {
