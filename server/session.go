...
func (s *Session) getGrpTopic() *string {
	s.subsLock.RLock()
	defer s.subsLock.RUnlock()
	for topicTemp := range s.subs {
		globals.logger.Infof("nodeForTopic: range sess.subs topic %s", topicTemp)
		if strings.Contains(topicTemp, "grp") {
			return &topicTemp
		}
	}
	return nil
...
func (s *Session) queueOut(msg *ServerComMessage) bool {
	...
	case <-time.After(time.Microsecond * 50):
		s.logger.Info("s.queueOut: timeout", s.sid)
		return false
	}
	return true
}
...
func (s *Session) queueOutBytes(data []byte) bool {
	...
	case s.send <- data:
	case <-time.After(time.Microsecond * 50):
		s.logger.Info("s.queueOutBytes: timeout", s.sid)
		return false
	}
	return true
}
...
func (s *Session) dispatchRaw(raw []byte) {
	...
	s.logger.Infof("in: '%s%s' ip='%s' sid='%s' uid='%s'", toLog, truncated, s.remoteAddr, s.sid, s.uid)
...
func (s *Session) dispatch(msg *ClientComMessage) {
	...
	if msg.from == "" {
		...
		s.logger.Info("s.dispatch: non-root asigned msg.from", s.sid)
		return
	}
	...
	checkVers := func(m *ClientComMessage, handler func(*ClientComMessage)) func(*ClientComMessage) {
		return func(m *ClientComMessage) {
			if s.ver == 0 {
				s.logger.Info("s.dispatch: {hi} is missing", s.sid)
	...
	checkUser := func(m *ClientComMessage, handler func(*ClientComMessage)) func(*ClientComMessage) {
		return func(m *ClientComMessage) {
			if msg.from == "" {
				s.logger.Info("s.dispatch: authentication required", s.sid)
	...
	default:
		...
		s.logger.Info("s.dispatch: unknown message", s.sid)
		return
	}
...
func (s *Session) leave(msg *ClientComMessage) {
	...
	if sub := s.getSub(expanded); sub != nil {
		s.logger.Info("leave", expanded, ",attached")
	...
	} else if globals.cluster.isRemoteTopic(expanded, msg.from) {
		// The topic is handled by a remote node. Forward message to it.
		if err := globals.cluster.routeToTopic(msg, expanded, s); err != nil {
			s.logger.Info("s.leave:", err, s.sid)
			s.queueOut(ErrClusterUnreachable(msg.id, msg.topic, msg.timestamp))
		}
	} else if !msg.Leave.Unsub {
		s.logger.Info("leave", expanded, "not attached")
		...
		s.logger.Info("s.leave:", "must attach first", s.sid)
...
func (s *Session) publish(msg *ClientComMessage) {
	...
	if sub := s.getSub(expanded); sub != nil {
		...
		// The topic is handled by a remote node. Forward message to it.
		if err := globals.cluster.routeToTopic(msg, expanded, s); err != nil {
			s.logger.Info("s.publish:", err, s.sid)
			s.queueOut(ErrClusterUnreachable(msg.id, msg.topic, msg.timestamp))
		}
	} else {
		// Publish request received without attaching to topic first.
		s.queueOut(ErrAttachFirst(msg.id, msg.topic, msg.timestamp))
		s.logger.Info("s.publish:", "must attach first", s.sid)
	}
}
...
func (s *Session) hello(msg *ClientComMessage) {
	...
	if s.ver == 0 {
		...
		if s.ver == 0 {
			s.logger.Info("s.hello:", "failed to parse version", s.sid)
		...
		if versionCompare(s.ver, minSupportedVersionValue) < 0 {
			...
			s.logger.Info("s.hello:", "unsupported version", s.sid)
			return
		}
		...
		if !s.uid.IsZero() {
			if err := store.Devices.Update(s.uid, s.deviceID, &types.DeviceDef{
				...
				s.logger.Info("s.hello:", "database error", err, s.sid)
		...
		s.logger.Info("s.hello:", "version cannot be changed", s.sid)
		return
	}
...
func (s *Session) acc(msg *ClientComMessage) {
	...
	if msg.Acc.Token != nil {
		if !s.uid.IsZero() {
			...
			s.logger.Info("s.acc: got token while already authenticated", s.sid)
			return
		}
		...
		if err != nil {
			...
			s.logger.Info("s.acc: invalid token", err, s.sid)
			return
		}
	}
...
func (s *Session) login(msg *ClientComMessage) {
	...
	if handler == nil {
		s.logger.Info("s.login: unknown authentication scheme", msg.Login.Scheme, s.sid)
	...
	if err != nil {
		s.logger.Info("s.login: failed to validate credentials", err, s.sid)
		s.queueOut(decodeStoreError(err, msg.id, "", msg.timestamp, nil))
	} else {
...
func (s *Session) onLogin(msgID string, timestamp time.Time, rec *auth.Rec, missing []string) *ServerComMessage {
	...
	if len(missing) > 0 {
		...
		if len(rec.Tags) > 0 {
			s.logger.Info("Resetting user's tags", normalizeTags(rec.Tags))

			if err := store.Users.UpdateTags(rec.Uid, normalizeTags(rec.Tags), true); err != nil {

				s.logger.Info("failed to update user's tags", err)
			}
		}
		...
		if s.deviceID != "" {
			if err := store.Devices.Update(rec.Uid, "", &types.DeviceDef{
				...
				s.logger.Info("failed to update device record", err)
			}
		}
	}
...
func (s *Session) get(msg *ClientComMessage) {
	...
	if meta.what == 0 {
		...
		s.logger.Info("s.get: invalid Get message action", msg.Get.What)
	} else if sub != nil {
		sub.meta <- meta
	} else if globals.cluster.isRemoteTopic(expanded, msg.from) {
		// The topic is handled by a remote node. Forward message to it.
		...
		s.logger.Info("s.get: subscribe first to get=", msg.Get.What)
		s.queueOut(ErrPermissionDenied(msg.id, msg.topic, msg.timestamp))
	} else {
...
func (s *Session) set(msg *ClientComMessage) {
	...
	if meta.what == 0 {
		s.queueOut(ErrMalformed(msg.id, msg.topic, msg.timestamp))
		s.logger.Info("s.set: nil Set action")
		...
		s.logger.Info("s.set: can Set tags for subscribed topics only")
		s.queueOut(ErrPermissionDenied(msg.id, msg.topic, msg.timestamp))
	} else {
...
func (s *Session) del(msg *ClientComMessage) {
	...
	s.logger.Infof("s.del in: Del %v action %v", msg.Del.What, s.sid)
	if what == 0 {
		s.queueOut(ErrMalformed(msg.id, msg.topic, msg.timestamp))
		s.logger.Infof("s.del: invalid Del action %v, sid %v", msg.Del.What, s.sid)
		return
	}
	...
	if sub != nil && what != constMsgDelTopic {
		...
		s.logger.Infof("s.del: topic %v, sid %v", expanded, s.sid)
		sub.meta <- &metaReq{
			topic: expanded,
			pkt:   msg,
			sess:  s,
	...
	} else if sub == nil && globals.cluster.isRemoteTopic(expanded, msg.from) {
		// The topic is handled by a remote node. Forward message to it.
		if err := globals.cluster.routeToTopic(msg, expanded, s); err != nil {
			s.queueOut(ErrClusterUnreachable(msg.id, msg.topic, msg.timestamp))
		}
		...
		s.queueOut(ErrAttachFirst(msg.id, msg.topic, msg.timestamp))
		s.logger.Infof("s.del: invalid Del action %v while unsubbed, sid %v", msg.Del.What, s.sid)
	}
}
...
func (s *Session) expandTopicName(msg *ClientComMessage) (string, *ServerComMessage) {
	...
	if msg.topic == "" {
		s.logger.Info("s.etn: empty topic name", s.sid)
	...
	if msg.topic == "me" {
		...
		if uid2.IsZero() {
			...
			s.logger.Info("s.etn: failed to parse p2p topic name", s.sid)
			return "", ErrMalformed(msg.id, msg.topic, msg.timestamp)
		} else if uid2 == uid1 {
			// Use 'me' to access self-topic
			s.logger.Info("s.etn: invalid p2p self-subscription", s.sid)
